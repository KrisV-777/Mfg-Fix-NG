...
| 13.3 | ~~ | ~~**BlinkDownAndWait1 stuck state**~~ **BY DESIGN** -- the engine sets `BlinkDownAndWait*` stages combined with `unk21A=true` to keep eyes closed for dead/sleeping/unconscious NPCs. The "stuck" behavior is the intended mechanism. A previous attempt to add a `BlinkUp` transition on timer expiry caused dead bodies to blink and was reverted | EyesBlinkingUpdate | Working as engine intends |
