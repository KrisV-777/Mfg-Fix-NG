...
namespace MfgFix
{
  void Init()
  {
	...
	if (REL::Module::IsVR()) {
	  REL::safe_fill(REL::Offset(0x3d3670).address() + REL::VariantOffset(0x4B, 0x4A, 0x4B).offset(), 0x90, REL::VariantOffset(0x09, 0x0D, 0x09).offset());
	  // fix papyrus function SetExpressionOverride that didn't work with aiMood 0
	  // first offset seems the same
	  REL::safe_write(REL::Offset(0x9869d0).address() + 0x001A, static_cast<std::uint16_t>(0x9001));
	  // second is different
	  REL::safe_write(REL::Offset(0x9869d0).address() + REL::VariantOffset(0x29, 0x2A, 0x29).offset(), static_cast<std::uint8_t>(0x10));

	  // disable code that sometimes cause ctd because of missing check for null parent node
	  REL::safe_fill(REL::Offset(0x3e82f0).address() + REL::VariantOffset(0x2E3, 0x0395, 0x2E3).offset(), 0x90, 0x0F);
	} else {
